...
=======
  Fixpoint getNStateSpec n r (ls: SystemStream r) :=
>>>>>>> ae880597d5babd3d042c9e0a2604e23f2f182ba8
    match ls with
      | SCons x _ _ ls' => match n with
                             | 0 => x
                             | S m => getNStateSpec m ls'
...
=======
  Section ForOneStream.
    Variable anyInitRest: Rest.
    Variable stm: SystemStream anyInitRest.
    Variable initIs: Addr -> Proc -> nat.
...
>>>>>>> ae880597d5babd3d042c9e0a2604e23f2f182ba8
    ...
    Definition getNSystem n := getNSystem' stm initIs n.
    ...
    Lemma isInitGe n:
      match respFn n with
        | Some (Build_Resp a p i _) => initIs a p <= i
        | None => True
      end.
    Proof.
      pose proof (fun a p d w d' i => @getNSystem'_monotone a p d w d' i n
                                                            anyInitRest stm initIs).
      unfold respFn, getNSystem.
      pose proof (getPf' n stm initIs) as sth.
      destruct (getNSystem' stm initIs n).
      simpl in H.
      destruct (getInfo s).
      destruct p, p, p, p.
      specialize (H a p d1 d0 d).
      destruct o.
      specialize (H n0).
      intuition.
      intuition.
    ...
    Variable anyInitData: Addr -> Data.
    Variable sa: StoreAtomicity anyInitData reqFn respFn.
    ...
    Definition AtomicList := TransList AtomicTrans (Build_State anyInitData initIs).
    ...
    Definition latestAtomValue a t :=
      (mem (getTransSt getTransNext t) a = anyInitData a /\
       noAtomStore a 0 t) \/
      (exists tm,
    ...
    Definition atomNoPrevNonSt a t :=
      noAtomStore a 0 t /\
      mem (getTransSt getTransNext (S t)) a = anyInitData a /\
      noCurrAtomStore a t.
...
>>>>>>> ae880597d5babd3d042c9e0a2604e23f2f182ba8
    ...
    Section PrevMatch.
      ...
      Proof.
        assocResp.
        pose proof (isInitGe t) as mur1.
        case_eq (respFn t).
        intros r caseR; destruct r; simpl in *.
        rewrite caseR in mur1.
        intros nextGt.
        pose proof (allAtomPrev _ mur1 nextGt) as [t' [t'_lt_t allPrev]].
